...
int main()
{
	...
	//auto code1 = fs.OpenFile(std::string("MyFile"));
	static_cast<void>(fs.OpenFile(std::string("MyFile")));

	//fs.SendEmail(std::string("me@gmail.com"), std::string("Hi"));
	[[maybe_unused]] auto code2 = fs.SendEmail(std::string("me@gmail.com"), std::string("Hi"));

	//fs.SystemCall(std::string("SysFile"));
	//auto code3 = fs.SystemCall(std::string("SysFile"));
	[[maybe_unused]] auto code3 = fs.SystemCall(std::string("SysFile"));
